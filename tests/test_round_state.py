...
def test_doubled_or_variant_footer_diagnostic_stays_ineligible(suffix):
    observed: list[str] = []

    assert _recover((_issue_comment(str(_diagnostic_body()) + suffix, comment_id=900),), observed) is None
    # A doubled footer is stripped once at ingestion and then rejected by the
    # exact-canonical decoder; the decoder itself never strips.
    assert observed == []
