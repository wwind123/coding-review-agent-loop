...
| Workflow transaction publication seam (#827, stage B, writers unwired) | `workflow_transaction_publication.py` | The transaction boundary: sole writer of transaction records and the records they bind. `publish_transition` reads both surfaces through the authenticated reader, reconciles prepared-only siblings *below* the strict lineage resolver (lowest prepared ID wins; the observer appends the abort), adopts a stored prepared intent, aborts and re-prepares an obsolete intent, publishes each `reissued` entry read-first (byte-exact actor adoption, else one verified write), re-reads, and appends the terminal record. `require_committed_transaction` is the fail-closed, never-writing gate (committed live-head transaction, entries, initial coder round, checkpoint); a legacy-era PR returns none and keeps the version-1 checks. `discover_canonical_issue_pr` is the authority reader for issue-only callers; `route_issue_publication` hands an interrupted publication to the seam as a record-less `Recoverable`/`RecoverableSuccessor` route (private filtered views only for a sibling group). `WorkflowTransactionError`s are recoverable (`NoLiveHeadAuthority` via `resolve_round_authority`) or integrity errors that stop the round. The managed-CI entry goes through `AuthorizationEntryCodec`. Advisory and audit comments (scheduling audits, parent phase handoffs) are outside it. Gate consumers: `authenticate_canonical_issue_pr` (issue resume, phase progress), every merge, the qualification snapshot, and the per-round head binding, which gates checkpoint, interrupted-round, and approval reuse (none under `NoLiveHeadAuthority`). They, loop entry, and each dispatched-coder head commit a `head-advance` successor from the committed intent first (unmanaged only). No issue-path writer uses the seam yet. |
| Bound managed-CI authorization (#827, stage B, partly wired) | `managed_ci_bound_authorization.py` | A transaction's managed-CI entry: a registered PR-comment-only record with the version-1 authorization fields plus transaction ID, grant anchor, optional upgrade source, and the kinds `plan-rebind` and `ordinary-release`. The granted generation hashes repository, issue, PR, base, actor ID, protection, waiver, grant anchor, and plan hash (never head, kind, nonce, or live label event); the released generation excludes the plan. `validate_bound_authorization` alone judges a bound record (envelope, scope, plan, recomputed generation, kind shape in a native or an upgrade branch, nonce); the upgrade branch re-derives its source with the pure `select_upgrade_source` rule and walks the legacy chain. `BoundAuthorizationCodec` implements `AuthorizationEntryCodec`, so seam, gate, and router apply that one rule. Accessor modes are pure reads applying that rule first: builder mode returns a `BuilderAuthorizationView` (`effective`, `nearest_granted`, `pending`, diagnostics-only `superseded`; no selection by head), consumer mode only the latest committed granted record at the live head, pending mode a prepared-only record as input only. `managed_ci.read_managed_ci_authorizations` is the only scan of unbound authorizations: version-1 semantics in the legacy era, nothing once the PR has an actor-authored version-2 record, so version-1 publishers refuse to write and the resume audit grants nothing. The dispatch-time label release first runs the `before_label_release` hook, committing the released successor on a granted PR. No grant builder emits a bound payload yet. |
