...
| Workflow transaction publication seam (#827, stage B, not yet wired) | `workflow_transaction_publication.py` | The transaction boundary: the only module that writes transaction records and the records a transaction binds. `publish_transition` reads both surfaces through the authenticated reader, reconciles prepared-only siblings *below* the strict lineage resolver (lowest prepared comment ID wins; the observer appends the `sibling-canonical` abort), adopts a stored prepared intent instead of rebuilding it, aborts an obsolete intent and re-prepares, publishes each `reissued` entry read-first (byte-exact actor-authored adoption, else one read-back-verified write through one choke point), re-reads, and appends the terminal record. `require_committed_transaction` is the fail-closed, never-writing gate (committed transaction for the live head, every entry re-verified, initial-coder-round rule, scheduler checkpoint); a legacy-era PR returns no transaction and keeps the version-1 checks. `discover_canonical_issue_pr` is the authority reader for issue-only callers; `route_issue_publication` is the non-authority router that hands an interrupted publication to the seam as a record-less `Recoverable`/`RecoverableSuccessor` route, with router-private filtered views only for a true sibling group. Failures are `WorkflowTransactionError`s: a recoverable class (`NoLiveHeadAuthority` via `resolve_round_authority`) and an integrity class that stops the round. The managed-CI authorization entry is reached through the `AuthorizationEntryCodec` protocol. Advisory and audit comments (scheduling audits, parent phase handoffs) are outside the transaction. No orchestrator or phase-progress call site uses the seam yet, so production emits no version-2 record. |
| Bound managed-CI authorization (#827, stage B, not yet wired) | `managed_ci_bound_authorization.py` | The managed-CI authorization entry of a workflow transaction: a registered PR-comment-only record carrying the version-1 authorization fields plus the transaction ID, the grant anchor, an optional upgrade source, and the two extra kinds `plan-rebind` and `ordinary-release`. The granted generation hashes repository, issue, PR, base, actor ID, protection, waiver, grant anchor, and plan hash (never head, kind, nonce, or live label event); the released generation excludes the plan. `validate_bound_authorization` is the only place a bound record is judged (envelope, scope, plan, recomputed generation, kind shape in a native or an upgrade branch, nonce); the upgrade branch re-derives its source with the pure `select_upgrade_source` rule and walks the legacy chain. `BoundAuthorizationCodec` implements the seam's `AuthorizationEntryCodec`, so the seam, gate, and router apply that one rule. The transaction-era accessor modes are pure reads that apply that rule first: builder mode returns a `BuilderAuthorizationView` (`effective`, `nearest_granted`, `pending`, diagnostics-only `superseded`; no selection by head), consumer mode only the latest committed granted record at the live head, and pending mode a prepared-only record as seam and selection input only. `managed_ci.read_managed_ci_authorizations` is the only scan of unbound authorizations: version-1 semantics in the legacy era, nothing once the PR carries an actor-authored version-2 record, so the version-1 publishers refuse to write and the resume audit grants nothing. No builder produces a bound payload yet, so none is emitted in production. |
