...
| Workflow transaction publication seam (#827, stage B, writers partly wired) | `workflow_transaction_publication.py` | The transaction boundary: sole writer of transaction records and the records they bind. `publish_transition` reads both surfaces through the authenticated reader, reconciles prepared-only siblings below the strict resolver (lowest prepared ID wins), adopts a stored prepared intent, aborts and re-prepares an obsolete intent, publishes each `reissued` entry read-first (actor adoption, else one verified write), then the terminal record. `require_committed_transaction` is the fail-closed, never-writing gate (committed live-head transaction, entries, coder round, checkpoint); a legacy-era PR returns none and keeps the version-1 checks. `discover_canonical_issue_pr` is the issue-only authority reader; `route_issue_publication` hands an interrupted publication to the seam as a record-less `Recoverable`/`RecoverableSuccessor` route. `WorkflowTransactionError`s are recoverable (`NoLiveHeadAuthority` via `resolve_round_authority`) or integrity errors that stop the round. The managed-CI entry goes through `AuthorizationEntryCodec`. Advisory and audit comments are outside it. Gate consumers: `authenticate_canonical_issue_pr` (issue resume, phase progress), every merge, the qualification snapshot, and the per-round head binding, which gates checkpoint, interrupted-round, and approval reuse (none under `NoLiveHeadAuthority`). They, loop entry, and each dispatched-coder head commit a `head-advance` successor from the committed intent first (unmanaged only). The PR loop reads a committed PR via `committed_pr_binding` (no v1 reads or writes), widens by a `closing-widening` successor, upgrades an unmanaged non-plan legacy PR that needs a write by one `initial` transaction, and at entry finishes a pending unmanaged transaction from its stored intent. The unmanaged direct fresh-PR issue writer publishes one `initial` transaction (handoff, contract, transaction-tagged initial coder round with no embedded contract copy); the other issue-path writers, managed issue PRs, and issue-command recovery routing are not yet on the seam. |
