...
| Workflow transaction publication seam (#827, stage B, not yet wired) | `workflow_transaction_publication.py` | The transaction boundary: the only module that writes transaction records and the records a transaction binds. `publish_transition` reads both surfaces through the authenticated reader, reconciles prepared-only siblings over grouped transactions *below* the strict lineage resolver (lowest prepared comment ID wins; the observer appends the `sibling-canonical` abort), adopts a stored prepared intent instead of rebuilding it, aborts an obsolete intent and re-prepares from the last committed transaction, publishes each `reissued` entry read-first (byte-exact adoption from the authenticated actor, otherwise one read-back-verified write through a single write choke point), re-reads, and appends the terminal record. The PR contract supersession is a pure function of the contract resolved for the predecessor chain. `require_committed_transaction` is the fail-closed, never-writing gate (committed canonical transaction for the live head, every entry re-verified, initial-coder-round rule, scheduler checkpoint); a legacy-era PR returns no transaction and keeps the version-1 checks. `discover_canonical_issue_pr` is the authority reader for callers that know only the issue; `route_issue_publication` is the non-authority router that returns an interrupted publication to the seam as a record-less `Recoverable`/`RecoverableSuccessor` route, using router-private filtered views only for a true prepared-only sibling group. Failures are `WorkflowTransactionError`s split into a recoverable class (mapped to `NoLiveHeadAuthority` by `resolve_round_authority`) and an integrity class that stops the round. The managed-CI authorization entry is reached through the `AuthorizationEntryCodec` protocol so `managed_ci` never imports this module. Advisory and audit comments (scheduling audits, parent phase handoffs) are outside the transaction. No orchestrator, managed-CI, or phase-progress call site uses the seam yet, so no version-2 record is emitted in production. |
| Bound managed-CI authorization (#827, stage B, not yet wired) | `managed_ci_bound_authorization.py` | The managed-CI authorization entry of a workflow transaction: a separate registered PR-comment-only record that carries the version-1 authorization fields plus the transaction ID, the grant anchor, an optional upgrade source, and the two extra kinds `plan-rebind` and `ordinary-release`; the version-1 record and parser are untouched. The granted generation hashes the repository, issue, PR, base, actor ID, protection, waiver, grant anchor, and approved-plan hash (never the head, kind, nonce, or live label event); the released generation excludes the plan. `validate_bound_authorization` is the only place a bound record is judged (envelope, scope, plan, recomputed generation, kind shape in a native or an upgrade branch, nonce) and reads durable author-authenticated records only; the upgrade branch re-derives its source with the pure `select_upgrade_source` rule and walks the legacy chain. `BoundAuthorizationCodec` implements the seam's `AuthorizationEntryCodec`, so the seam, gate, and router apply that one rule and `managed_ci` imports neither module. The transaction-era accessor modes are pure reads over an authenticated PR view and its resolved lineage, each applying that rule before returning a record: builder mode returns a `BuilderAuthorizationView` (`effective`, `nearest_granted`, `pending`, and diagnostics-only `superseded`, with no selection by head), consumer mode returns only the latest committed transaction's granted record at the live head and never an `ordinary-release` or pending record, and pending mode exposes a prepared-only transaction's record as seam and selection input only. No builder produces a bound payload and no production consumer calls the accessor yet, so none is emitted or honored in production. |
