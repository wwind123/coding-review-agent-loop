...
# Claude Code cloud sessions append this exact suffix server-side to every
# comment they post, whoever wrote the text (#1043).  It carries no marker, so
# tolerating it cannot add authority.  Only this one byte-exact suffix is
# tolerated, and only once: a stored body is either the posted body or the
# posted body plus this footer.  The footer is removed exactly once, at the
# boundary where a raw GitHub comment body enters a strict protocol reader
# (``_authenticated_comment_from_rest`` in github.py, ``_normalized_comment_body``
# in managed_ci.py, and ``recover_plan_validation_diagnostic`` in
# round_state.py).  Strict parsers and decoders never strip, so a doubled
# footer still reaches them with one footer attached and is rejected.
KNOWN_HOST_COMMENT_FOOTER = "\n\n---\n_Generated by [Claude Code](https://claude.ai/code)_"


def strip_known_host_footer(text: str) -> tuple[str, bool]:
    """Remove one trailing known host footer; report whether it was present.

    Never loops, never trims whitespace, and never touches earlier text.
    """
    if text.endswith(KNOWN_HOST_COMMENT_FOOTER):
        return text[: -len(KNOWN_HOST_COMMENT_FOOTER)], True
    return text, False


def stored_body_matches_posted(stored: object, posted: str) -> tuple[bool, bool]:
    """Return ``(matches, footer_observed)`` for one read-back comparison.

    The stored body matches only when it equals the posted body exactly or the
    posted body followed by exactly one known host footer.
    """
    if not isinstance(stored, str):
        return False, False
    if stored == posted:
        return True, False
    if stored == posted + KNOWN_HOST_COMMENT_FOOTER:
        return True, True
    return False, False
...
EXCLUDED_PROTOCOL_LITERALS = frozenset(
    {
        ...
        "AGENT_LOOP_MANAGED_CI_HOST_FOOTER_V1",
