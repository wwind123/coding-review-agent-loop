...
additional matrix-level audit entry. Its operation must be `change`, `split`,
or `merge`, and its `row_ids` must list the complete union of row IDs in the
prior and revised matrices (use `["matrix"]` when both matrices have no rows).
This complete-scope entry is separate from precise row add/change/retire/split/
merge entries and may intentionally overlap their row IDs.
"""
