...

{
  ...
  "human_requirement_dispositions": [
    {"requirement_id": "Requirement hr-<digest>", "disposition": "blocked", "evidence": "<why it cannot be completed>"}
  ],
  "human_requirements": {
    "addressed_ids": [],
...

{
  ...
  "plan_steps": ["Update the parser.", "Add regression tests."],
  "human_requirement_dispositions": [
    {"requirement_id": "Requirement hr-<digest>", "disposition": "addressed", "evidence": "Covered by the revised plan."}
  ]
}
...
- Requirement hr-<digest>: <how the revised plan addresses this signed human requirement, if present in the original>
<!-- AGENT_PLAN_STATE: blocking -->
-- <Coder Name>
...
- addressed_items, remaining_items -> reviewer ITEM IDs only: short slugs matching [A-Za-z0-9][A-Za-z0-9._-]*
  ...
  NEVER put invented or positional human requirement labels here — use only exact surfaced `Requirement hr-<digest>` labels.
- human_requirements.addressed_ids -> exact surfaced stable human REQUIREMENT LABELS like "Requirement hr-<digest>"
...
- In mixed cases, keep valid surfaced stable requirement labels and drop invalid extras.
...
```json
{
  ...
  "remaining_items": [],
  "human_requirement_dispositions": [
    {"requirement_id": "Requirement hr-<digest>", "disposition": "blocked", "evidence": "The required service is unavailable."}
  ],
  "human_requirements": {
    "addressed_ids": [],
    "checked_discussion_directly": false
...
- **Requirement hr-<digest>**: Implemented.
...
- Requirement hr-<digest>: The revised plan preserves backward compatibility.
...
- If surfaced signed labels include a stable `Requirement hr-<digest>` label, include one evidenced disposition for it and put it in addressed_ids only when that disposition is `addressed`; blocked dispositions require blocking state.
...
The repair context surfaced: Requirement hr-<digest>.
...
def _coder_followup_required_items_instruction(
    ...
    return (
        ...
        "Do not put human requirement labels such as `Requirement hr-<digest>` in these arrays.\n"
    )
...
def _reviewer_human_requirements_instruction(
    ...
    if expected_kind == "plan_review":
        disposition_instruction = (
            ...
            "with the exact stable surfaced requirement ID, disposition `addressed`, `blocked`, or `not-applicable`, "
