...
With no override, it uses the ordered fallback chain `Gemini 3.8 Flash (High)` →
`Gemini 3.7 Flash (High)` → `Gemini 3.6 Flash (High)` → `Gemini 3.1 Pro (High)`. Use `--model MODEL` for the legacy single-model
